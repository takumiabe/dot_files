...
(defcustom sql-smie-indent-basic 4
...
(defconst sql-smie-tokens
  '(("SELECT" . ("select" "select distinct"))
    ("DELETE" . ("delete"))
    ("CREATE" . ("create table" "create temporary table" "create view"))
    ("ALTER"  . ("alter table"))
    ("INSERT" . ("insert into" "upsert into"))
...
(defun sql-smie-rules (kind token)
  (pcase (cons kind token)
    (`(:elem . basic) sql-smie-indent-basic)
    (`(:after . ",") (smie-rule-separator kind))
    (`(:before . ",") (smie-rule-separator kind))
    (`(:after . "ON") sql-smie-indent-basic)
    (`(:after . ";") (cons 'column 0))
    )
  )
